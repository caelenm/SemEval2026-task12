...
# LLM setup
models = ["deepseek-v3.2-exp-thinking-original"]
seen_topics_dict = {}
def generate_llm_prediction(question_data, topic_docs, model):
    ...
    if rel_docs != None:
        RelevantDocs = {rel_docs[0]['title'],rel_docs[0]['content']}
    if rel_docs == None:
        RelevantDocs = []
    if COT == 1:
    ...
    Instructions:
    {thinkingPrompt} 
    Based solely on the information provided, select ANY OF (A, B, C, or D) that best explains the cause.
    Evaluation: you will get 1 point for an exactly correct guess (perfect match, e.g. guess A,C = answer A,C), 0.5 points for a partially correct guess
    (one or more matching letter. e.g. guess A , but correct was A, B, C), and zero points for no match. 
    Requirement: Output strictly ONLY the letters you predict separated by commas. Do not output any other text.
    """
    print(prompt)
        # 2. Call the API
    try:
